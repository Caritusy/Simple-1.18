...
void ConfigManager::loadConfig(std::string name, bool create) {
	...
	if (configExists || create) {
		...
		if (Game.getLocalPlayer() != nullptr) {
			...
			Game.getGuiData()->displayClientMessageF("[%sSimpleMod%s] %sSuccessfully %s config %s%s%s!", GOLD, WHITE, GREEN, !configExists ? "created" : "loaded", GRAY, name.c_str(), GREEN);
			if (!helpedUser && name != "default") {
				helpedUser = true;
				Game.getGuiData()->displayClientMessageF("[%sSimpleMod%s] %sEnter \"%s%cconfig load default%s\" to load your old config!", GOLD, WHITE, YELLOW, WHITE, cmdMgr->prefix, YELLOW);
			}
		}
	}
	else {
		if (Game.getLocalPlayer() != nullptr)
			Game.getGuiData()->displayClientMessageF("[%sSimpleMod%s] %sCould not load config %s%s%s!", GOLD, WHITE, RED, GRAY, name.c_str(), RED);
	}
...
void ConfigManager::saveConfig() {
	...
	Game.getClientInstance()->getGuiData()->displayClientMessageF("%s%s%s%sSaving config %s", BOLD, UNDERLINE, ITALIC,GREEN, currentConfig.c_str());
