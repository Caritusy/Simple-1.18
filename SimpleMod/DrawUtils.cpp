...
ImVec2 DrawUtils::getTextWidth(std::string* textStr, float textsize) {
	ImGuiIO& io = ImGui::GetIO();
	return  io.Fonts->Fonts[0]->CalcTextSizeA(textsize, FLT_MAX, 0.0f, textStr->c_str());
}
...
void DrawUtils::drawLine(const Vec2& start, const Vec2& end, ImU32 col, float lineWidth) {
	ImGui::GetBackgroundDrawList()->AddLine(ImVec2(start.x, start.y), ImVec2(end.x, end.y), col, lineWidth);
}
...
void DrawUtils::fillRectangle(const Vec4& pos, const ImU32 col, float alpha) {
	ImGui::GetBackgroundDrawList()->AddRectFilled(ImVec2(pos.x, pos.y), ImVec2(pos.z, pos.w), col);
}
