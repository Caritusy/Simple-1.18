...
std::string Arraylist::getModuleName()
{
	return ("Arraylist");
}
struct IModuleContainer {
	...
	IModuleContainer(std::shared_ptr<IModule> mod) {
		...
		const char* moduleNameChr = mod->getModuleName().c_str();
		...
		if (!enabled && *pos == Vec2(0.f, 0.f))
			shouldRender = false;
		textWidth = DrawUtils::getTextWidth(&moduleName, hudMod->scale).x;
	}
...
void Arraylist::onPostRender(){
	...
	Vec2 windowSizeReal = Game.getClientInstance()->getGuiData()->windowSize;
	...
	for (const auto& container : modContainerList) {
		...
		if (alpha > 0) {
			DrawUtils::fillRectangle(rectPos, ImColor(12,12,12), alpha);
		...
		DrawUtils::drawText( Vec2(textPos.x- 255, textPos.y-255 ), & textStr, arrayColor, 1, 0, textSize);
