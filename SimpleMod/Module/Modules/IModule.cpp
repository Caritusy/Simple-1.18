...
void IModule::registerTextSetting(std::string name,std::string* str){
	SettingEntry* setting = new SettingEntry();
	setting->valueType = ValueType::TEXT_T;

	//setting->value = reinterpret_cast<SettingValue*>(str);  // Actual value

	//setting->defaultValue = new SettingValue();
	//setting->defaultValue->text = str;

	strcpy_s(setting->name, 20, name.c_str());
...
void IModule::onKeyUpdate(int key, bool isDown){
	if (key == getKeybind()) {
		if (isFlashMode())
			...
			toggle();
			Game.getGuiData()->displayClientMessageF("[%sSimpleMod%s] >> %s%s %s!", GOLD, WHITE, isEnabled() ? GREEN : RED, getModuleName().c_str(), isEnabled() ? u8"enabled ✓" : u8"disabled ✕");
		}
	}
}
...
void IModule::onSendPacket(Packet*,bool&)
{
}
		...
		for (auto it = this->settings.begin(); it != this->settings.end(); ++it) {
			...
			if (obj.contains(sett->name)) {
				...
				try {
					...
					LOG(INFO)<<"Config Load Error(Enum) "<< this->getRawModuleName().c_str()<<e.what();
				}
			}
		}
		if (this->enabled)
...
void IModule::onSaveConfig(void* confVoid) {
	...
	for (auto sett : this->settings) {
		...
		case ValueType::BOOL_T:
			...
			break;
		case ValueType::TEXT_T:
			obj.emplace(sett->name, sett->value->text->c_str());
			break;
		case ValueType::ENUM_T:
			obj.emplace(sett->name, sett->value->_int);
			break;
...
//	Game.getGuiData()->displayClientMessageF("[%sSimpleMod%s] >> %s%s%s!", GOLD, WHITE, this->isEnabled() ? GREEN : RED, this->getModuleName(), (this->isEnabled() ? "[OFF]" : "[ON]"));
}
